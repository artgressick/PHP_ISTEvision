...
function errEmailExists(name,message) {
	...
	if(ajax) {
		ajax.open("GET", "ajax_functions.php?postType=checkemail&email="+ document.getElementById(name).value);
...
function checktimes(date,tb, te, id, location_id, message) {
	ajax = startAjax();
	var address = "ajax_functions.php?postType=checktimes&id="+id+"&date="+date+"&tb="+tb+"&te="+te+"&location_id="+location_id;
//	alert(address);
	if(ajax) {
