...
if ($_POST['confirm']=='1') {
	...
	if ($_SESSION['user_id'] > 0 && isset($_SESSION['videofile']) && $_SESSION['videofile'] != '') {
		...
		$vid_name = $_SESSION['videofile']; //This is from the FLVRecorder
		
		//Some fill in the gap information
		$size = "100000";
		$extension = "flv";
		...
		$query = "INSERT INTO videos SET
			...
			size = '".$size."',
			filename = '".$vid_name."',
			processed_file = '".substr($vid_name,0,-4)."',
			flv_status = 1,
...
*/		
		...
		if ($result) {
			...
			$subject = 'TESTING KIOSK New Video Created';
			$message = '
<html>
<head>
...
//			mail($to, $subject, $message, $headers, '-fwebmaster@istevision.org');
			...
			header ("Location: error-form.php"); //throw an error
			exit();
		}
