...
function setErrorMsg(message) { document.getElementById('errors').innerHTML += "<div class='ErrorMessage'>" + message + "</div>"; window.scrollTo(0,0); }

function errEmpty(name,message,array,max) {
	if(array != "array" && array != "extension") {
	...
	} else if (array == "array" && array != "extension") {
		...
		var upload_file = document.getElementById(name).value;
		var ext = new Array("avi","wmv","mov","m4v","mp4");  //These are the allowed extensions
		var file=upload_file.split('.');
		var len = (file.length - 1);
		var file_ext = file[len].toLowerCase();
