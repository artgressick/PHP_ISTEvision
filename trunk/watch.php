<?php
	...
	$sql_query = "SELECT videos.id, processed_file, video_status_id, h264, title, description, first_name, last_name, DATE_FORMAT(videos.created_at, '%b %D, %Y') as day, iste_member,  DATE_FORMAT(videos.created_at, '%h:%i %p') as time, status, vid, admin_notes, videos.id, flv_status, jpg_status, ftp_status, meta_tags
	FROM videos
...
<script>
	...
	$f("player", "includes/flowplayer.commercial-3.0.7.swf",  {
		...
		playlist: [
			...
			//Play the video
			{
				url: 'http://bitcast-g.bitgravity.com/techit/<?= $videos['processed_file']; ?>.<?=($videos['h264'] == '0' ? 'flv':'mp4') ?>',
				//Google Analytics
