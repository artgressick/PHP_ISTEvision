...
<?php
	...
	while ($channels = mysql_fetch_array ($channel_result, MYSQL_ASSOC)) {
		...
		$sql_query = "SELECT processed_file, title, first_name, last_name, h264, vid, description, (select count(video_id) from views where videos.id = views.video_id) as watched, iste_member
		FROM videos
		JOIN customers ON videos.customer_id = customers.id
		WHERE ftp_status > 0 and video_status_id = 2 and videos.id in (SELECT video_id from video_channels where channel_id = ".$channels['id'].")
				...
				<tr>
					<td>
						<div class="slider-header">
								Most Popular <?= $channels['name'] ?> <a href="channel.php?c=<?= $channels['cid'] ?>&v=4"><img src="images/goto-channel.png" alt="Go to channel" width="97" height="18" class="goto-channel" border="0" /></a>
						</div>
